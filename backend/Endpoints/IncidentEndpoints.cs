    ...
    public static void MapIncidentEndpoints(this WebApplication app)
    {
        ...
        app.MapGet("/api/admin/incidents", async (HttpContext httpContext, AppDbContext db, int? safehouseId, int? residentId, string? severity, bool? resolved, string? sortBy = null, string? sortDir = null, int page = 1, int pageSize = 20) =>
        {
            ...
            var items = await ordered.Skip((page - 1) * pageSize).Take(pageSize)
                .Select(i => new { i.IncidentId, i.ResidentId, residentCode = i.Resident != null ? i.Resident.InternalCode : null, residentFirstName = i.Resident != null ? i.Resident.FirstName : null, residentLastName = i.Resident != null ? i.Resident.LastName : null, i.SafehouseId, i.IncidentDate, i.IncidentType, i.Severity, i.Description, i.ResponseTaken, i.ReportedBy, i.Resolved, i.ResolutionDate, i.FollowUpRequired })
                .ToListAsync();
        ...
        app.MapGet("/api/admin/incidents/{id}", async (int id, AppDbContext db) =>
        {
            var i = await db.IncidentReports.Where(x => x.IncidentId == id)
                .Select(x => new { x.IncidentId, x.ResidentId, residentCode = x.Resident != null ? x.Resident.InternalCode : null, residentFirstName = x.Resident != null ? x.Resident.FirstName : null, residentLastName = x.Resident != null ? x.Resident.LastName : null, x.SafehouseId, x.IncidentDate, x.IncidentType, x.Severity, x.Description, x.ResponseTaken, x.ReportedBy, x.Resolved, x.ResolutionDate, x.FollowUpRequired })
                .FirstOrDefaultAsync();
